...
if rag_enabled:
    try:
        rag = RAGManager(
            ...
            lakebase_client=memory.lakebase,
...
<body>
    <div class="container">
        ...
        <div class="controls">
            ...
            <button onclick="showSessionInfo()">ℹ️ Session Info</button>
        </div>
    ...
    <script>
        ...
        async function clearHistory() {
            ...
            if (response.ok) {
                ...
                addMessage('assistant', 'Conversation history cleared. Starting fresh! 🎯');
            }
        }
...
async def chat():
    """Handle chat messages with MLflow tracing"""
    data = request.json
    ...
    try:
        ...
        if rag_enabled and rag:
            try:
                with mlflow.start_span(name="rag_retrieve") as span:
                    ...
                    if docs:
                        ...
                        # Log RAG sources
                        mlflow.log_text("\n".join([d['source'] for d in docs]), "rag_sources.txt")
            except Exception as e:
                print(f"⚠️  RAG retrieval failed: {e}")
                mlflow.log_param("rag_error", str(e))
        
        # Build system prompt with context
        system_prompt = config["model"].get("system_prompt", "You are a helpful assistant.")
        if context:
...
{system_prompt}"""
        ...
        with mlflow.start_span(name="llm_call") as span:
            ...
            span.set_attribute("response_length", len(assistant_response))
            span.set_attribute("model", config["model"]["endpoint"])
        
        # Store assistant response
        memory.store_message(session_id, "assistant", assistant_response)
        
        # Log outputs
        mlflow.log_text(message, "user_message.txt")
        mlflow.log_text(assistant_response, "assistant_response.txt")
