...
✅ **Lakebase Integration** - Use managed PostgreSQL for state  
✅ **Session Management** - Track conversations by user/session  
✅ **Context-Aware Responses** - Reference past conversations  
✅ **RAG (Optional)** - Knowledge retrieval from documents  
✅ **Vector Search** - pgvector or Databricks Vector Search  
✅ **MLflow Tracing** - Track all interactions
...
conversations
├── id (serial)
├── session_id (varchar)  # Unique per user/session
...
# 📄 Checking documents in /Volumes/main/docs/source...
# 🔄 Indexing 42 new documents...
...
1. ✅ Creates Delta table: `main.default.docs_index_source`
2. ✅ Creates Vector Search index on the Delta table (TRIGGERED mode)
3. ✅ Indexes documents from UC Volume to Delta table
4. ✅ Vector Search auto-generates embeddings
5. ✅ Sync is triggered automatically after indexing (cost-effective)
...
- ✅ Scheduled (nightly, hourly, etc.)
- ✅ Monitored via Databricks Jobs UI
...
### **Recommended: Using Asset Bundles**

**1. Configure Lakebase credentials**

Edit `databricks.yml` or set variables:
```bash
export TF_VAR_lakebase_host="your-instance.cloud.databricks.com"
export TF_VAR_lakebase_database="agents"
export TF_VAR_lakebase_user="your-user"
export TF_VAR_lakebase_password="your-password"
```

**2. Deploy with bundle**
```bash
